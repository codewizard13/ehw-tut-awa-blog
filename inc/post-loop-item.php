...
// If we have an ID then generated a card,
if (isset($row['ID'])) {
  $card_id = $row['ID'];
  ...
  $_SESSION['cards'][$card_id]['post_title'] = $row['Title'];
  $_SESSION['cards'][$card_id]['pub_date'] = $date;
  $_SESSION['cards'][$card_id]['img_url'] = $img_url;
  $_SESSION['cards'][$card_id]['blurb'] = $row['Blurb'];
...
HTML;
  ...
  echo "<h4 class='error'>Card ID not found!</h4>";
}
